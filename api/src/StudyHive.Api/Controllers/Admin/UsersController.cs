...
namespace StudyHive.Api.Controllers.Admin;

/// <summary>
/// Shared/foundation: user administration. Not in the plan's section 11 table - it exists because W-25 needs it. Maintained by S1 with the rest of the foundation.
///
/// SCAFFOLD ONLY - owned by the foundation owner (S1), not implemented yet. Every action below returns 501 so the
...
public sealed class UsersController : ControllerBase
{
    ...
    private ObjectResult NotImplemented(string what) => Problem(
        ...
        detail: $"{what} is owned by the foundation owner (S1) and has not been built yet.");

    /// <summary>List users with their roles. Backs W-25.</summary>
    [HttpGet]
    [Authorize(Policy = "AdminOnly")]
    [ProducesResponseType(StatusCodes.Status501NotImplemented)]
    public IActionResult List([FromQuery] PageQuery query) => NotImplemented("Listing users");

    /// <summary>Change a user's role. W-25 requires a reason because the change writes an audit row.</summary>
    [HttpPut("{id:guid}/role")]
    [Authorize(Policy = "AdminOnly")]
    [ProducesResponseType(StatusCodes.Status501NotImplemented)]
    public IActionResult ChangeRole(Guid id) => NotImplemented("Changing a user role");

    /// <summary>Activate or deactivate an account.</summary>
    [HttpPut("{id:guid}/status")]
    [Authorize(Policy = "AdminOnly")]
    [ProducesResponseType(StatusCodes.Status501NotImplemented)]
    public IActionResult SetActive(Guid id) => NotImplemented("Changing account status");
}
